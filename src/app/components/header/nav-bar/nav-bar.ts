...
export class NavBar {
  ...
  /** Manages navigation and current route information */
  #router: RouterService = inject(RouterService);

  /** Global application state store */
  #store: Store = inject(Store);

  /** Authentication service for login state and user management */
  #auth: Auth = inject(Auth);

  /** Service responsible for displaying snackbar notifications */
  ...
  readonly #gameSettingsButton: Signal<DialogTriggerButton> = computed(() => {
    return {
      ...
      action: async () => {
        const dialogResult = await this.#dialog.open<GameSettings | 'CLOSE_EVENT'>('game_setting', {
          title: 'Game setup',
  ...
  readonly #navigateButton: Signal<DialogTriggerButton> = computed(() => {
    return {
      ariaLabel: this.#router.currentEndpoint() === 'account' ? 'account' : 'back',
      iconPath:
      ...
      action: () => {
        const url = this.#router.currentEndpoint() === 'account' ? 'tic-tac-toe' : 'account';
        const queryParams =
          this.#router.currentEndpoint() === 'account'
  ...
  readonly #saveButton: Signal<DialogTriggerButton> = computed(() => {
    return {
      ...
      condition: !!this.#auth.user() && this.#router.currentEndpoint() === 'tic-tac-toe',
    };
  });
  ...
  readonly #settingsButton: Signal<DialogTriggerButton> = computed(() => {
    return {
      ...
      action: async () => {
        const dialogResult = await this.#dialog.open<true | 'CLOSE_EVENT'>('setting', {
          title: 'Settings',
          content: 'setting',
        });
        if (dialogResult && dialogResult !== 'CLOSE_EVENT') {
  ...
  readonly #authButton: Signal<DialogTriggerButton> = computed(() => {
    return {
      ...
      action: async () => {
        ...
        } else if (dialogResult !== 'CLOSE_EVENT' && dialogResult && !dialogResult.rePassword) {
  ...
  readonly #logoutButton: Signal<DialogTriggerButton> = computed(() => {
    return {
      ...
      action: async () => {
        const dialogResult = await this.#dialog.open<true | 'CLOSE_EVENT'>('message', {
          title: 'Logout',
